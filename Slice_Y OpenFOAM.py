...
#Posizione File
png_output_directory = "D:\\Dynamis\\Script\\FotoScript\\Y\\"
casefoam = OpenFOAMReader(FileName='D:\\Dynamis\\CFD\\DPX\\case.foam')

#Posizione Camera
renderView1.CameraPosition = [0.5, -4.7, 0.5]
renderView1.CameraFocalPoint = [0.5, 6, 0.5]
...
y_pt = numpy.around(numpy.arange(0.05, 0.80, 0.05), decimals=2)
...
for i in range(len(x_pt)):
	...
	for j in range(len(campi)):
		...
		SetActiveSource(casefoam)
		Render()
		stringnorm = "_" + str(i) + "_y_" + str(y_pt[i])
